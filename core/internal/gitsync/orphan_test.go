...
func TestOrphanArchiveRequiresConfirmationAndNeverRemovesBeforeBackup(t *testing.T) {
	...
	require.NoError(t, err)
	require.Contains(t, backup.ArchivePath, "archives/")
	archivePath := filepath.Join(service.backupRoot, filepath.FromSlash(backup.ArchivePath))
...
func TestOrphanDeleteCreatesBackupAndRefusesDirtyEditor(t *testing.T) {
	...
	backup, err := service.store.GetBackup(result.Backup)
	require.NoError(t, err)
	require.NotContains(t, backup.ArchivePath, "archives/")
