...
interface RepositoryStackCatalog { repositoryId: string; branch: string; composePaths: string[]; }
interface Binding {
    ...
    autoSyncSelectionMode: "all" | "selected"; autoSyncComposePaths: string[];
...
function defaultRepositoryFolder(stackPath: string): string {
    const normalized = stackPath.trim().replaceAll("\\", "/").replace(/^\/+|\/+$/g, "");
...
export default function TabGit() {
    ...
    const [policyForm, setPolicyForm] = useState<GitPolicyForm>({profile: "compose_config", includes: "", excludes: ""});
    ...
    const [automationForm, setAutomationForm] = useState({enabled: false, autoReconcile: true, intervalMinutes: 15, autoSyncSelectionMode: "all" as "all" | "selected", autoSyncComposePaths: [] as string[], deployEnabled: false, deployNewStacks: false, deployRollback: false, deployComposePaths: [] as string[]});
    const automationSyncSelection = useMemo(() => new Set(automationForm.autoSyncComposePaths), [automationForm.autoSyncComposePaths]);
    ...
    const loadGitImportCatalog = async (repositoryId: string) => {
        setBusy("git-import-catalog");
        await (async () => {
            await api<RepositoryStatus>(`/repositories/${repositoryId}/pull`, {method: "POST"});
            const catalog = await api<RepositoryStackCatalog>(`/repositories/${repositoryId}/stack-catalog`);
            setGitImportCatalog(catalog);
            setGitImportComposePaths(new Set(catalog.composePaths));
            if (catalog.composePaths.length === 0) showWarning("No Compose stack was found on this repository branch.");
        })().catch((error) => {
            setGitImportCatalog(null);
            setGitImportComposePaths(new Set());
            showError(`Unable to inspect Git stacks: ${(error as Error).message}`);
        }).finally(() => setBusy(null));
    };
    ...
    const openGitStackImport = () => {
        ...
        if (repository) void loadGitImportCatalog(repository.id);
    };
    ...
    return <Box sx={{maxWidth: 1200, mx: "auto", p: {xs: 1, md: 3}}}>
            ...
            <Paper variant="outlined" sx={{borderRadius: 2, overflow: "hidden"}}>
                <Stack direction={{xs: "column", md: "row"}} sx={{p: 2.25, justifyContent: "space-between", gap: 2}}>
                    ...
                    <Stack direction="row" spacing={1}>
                        <Button variant="outlined" startIcon={<CloudDownloadOutlined/>} disabled={repositories.length === 0 || busy !== null} onClick={openGitStackImport}>Import from Git</Button>
                        ...
                        {bindings.map((binding) => <TableRow key={binding.id} hover>
                            ...
                            <TableCell sx={{minWidth: 190}}>
                                <Stack direction="row" spacing={.5} sx={{alignItems: "center"}}>
                                    ...
                                    {binding.autoSyncEnabled && <Tooltip title={`${binding.autoSyncComposePaths.length} of ${binding.selectedComposePaths.length} linked stack(s) are checked automatically`}><Chip size="small" variant="outlined" color={binding.autoSyncComposePaths.length === binding.selectedComposePaths.length ? "success" : "info"} label={`${binding.autoSyncComposePaths.length}/${binding.selectedComposePaths.length} auto`}/></Tooltip>}
        ...
        <Dialog open={bindingDialogOpen} onClose={() => busy === null && setBindingDialogOpen(false)} fullWidth maxWidth="md">
            ...
            <DialogContent dividers><Stack spacing={2} sx={{pt: .5}}>
                ...
                {stackTargets.length > 0 && <FormControl><InputLabel>Source folder</InputLabel><Select label="Source folder" value={stackTargets.some((target) => `${target.host}\n${target.path}` === `${bindingForm.host}\n${bindingForm.stackPath}`) ? `${bindingForm.host}\n${bindingForm.stackPath}` : ""} onChange={(event) => {
                    ...
                    if (target) {
                        setBindingForm({...bindingForm, host: target.host, stackPath: target.path, subPath: bindingForm.targetMode === "repository_folder" ? defaultRepositoryFolder(target.path) : "."});
        ...
        <Dialog open={gitImportDialogOpen} onClose={() => busy === null && setGitImportDialogOpen(false)} fullWidth maxWidth="md">
            <DialogTitle sx={{display: "flex", alignItems: "center", gap: 1}}><CloudDownloadOutlined/>Import stacks from Git</DialogTitle>
            <DialogContent dividers><Stack spacing={2} sx={{pt: .5}}>
                ...
                <FormControl><InputLabel>Repository and branch</InputLabel><Select label="Repository and branch" value={gitImportForm.repositoryId} onChange={(event) => {
                    ...
                    {repositories.map((repository) => <MenuItem key={repository.id} value={repository.id}>{repository.name} — {repository.defaultBranch}</MenuItem>)}
                </Select></FormControl>
                <Stack direction={{xs: "column", sm: "row"}} spacing={2}>
                ...
                {busy !== "git-import-catalog" && gitImportCatalog && <Box>
                    ...
                    <ComposePathSelector paths={gitImportCatalog.composePaths} selectedPaths={gitImportComposePaths} onChange={setGitImportComposePaths} selectedLabel="will be imported" unselectedLabel="not imported" maxHeight="38vh"/>
                </Box>}
                {busy !== "git-import-catalog" && gitImportCatalog?.composePaths.length === 0 && <Alert severity="warning">No compose.yml, compose.yaml, docker-compose.yml or docker-compose.yaml was found on this branch.</Alert>}
                <Alert severity="warning">The selected Git branch is pulled before discovery and again before import. Compose files are validated, the operation is confined to the destination folder, and any existing changed file is backed up before replacement.</Alert>
            </Stack></DialogContent>
            <DialogActions><Button onClick={() => setGitImportDialogOpen(false)} disabled={busy !== null}>Cancel</Button><Button variant="contained" startIcon={<CloudDownloadOutlined/>} onClick={() => void importGitStacks()} disabled={busy !== null || !gitImportForm.repositoryId || !gitImportForm.host.trim() || !gitImportForm.stackPath.trim() || gitImportComposePaths.size === 0}>{busy === "git-stack-import" && <CircularProgress size={16} sx={{mr: 1}}/>}Import and link</Button></DialogActions>
        </Dialog>
            ...
            <DialogContent dividers sx={{overflow: "hidden"}}><Stack spacing={2} sx={{pt: .5, height: "100%", minHeight: 0}}>
                ...
                <Box sx={{display: "grid", gridTemplateColumns: {xs: "1fr", lg: "390px minmax(0, 1fr)"}, gap: 2, minHeight: 0, flex: 1, overflow: {xs: "auto", lg: "hidden"}}}>
                    <Stack spacing={2} sx={{minWidth: 0, minHeight: 0, overflowY: "auto", pr: .5, "& .MuiAlert-message": {minWidth: 0, overflowWrap: "anywhere"}}}>
                        ...
                        <Typography variant="subtitle2">Advanced rules</Typography>
        ...
        <Dialog open={automationBinding !== null} onClose={() => busy === null && setAutomationBinding(null)} fullWidth maxWidth="md">
            ...
            <DialogContent dividers><Stack spacing={2} sx={{pt: .5}}>
                ...
                <Box><Typography variant="subtitle2" sx={{mb: 1}}>Automatic synchronization targets</Typography><ComposePathSelector key={`${automationBinding?.id || "automation"}-sync`} paths={automationBinding?.selectedComposePaths || []} selectedPaths={automationSyncSelection} onChange={(next) => {
                    const available = automationBinding?.selectedComposePaths || [];
                    const selected = available.filter((path) => next.has(path));
                    const allowed = new Set(selected);
                    setAutomationForm({...automationForm, autoSyncSelectionMode: selected.length === available.length ? "all" : "selected", autoSyncComposePaths: selected, deployComposePaths: automationForm.deployComposePaths.filter((path) => allowed.has(path))});
                }} selectedLabel="automatic" unselectedLabel="manual only" maxHeight="34vh"/></Box>
                <Alert severity="info">Stacks marked <strong>manual only</strong> stay linked and remain available for preview, push and pull actions. They are excluded only from scheduled polling. Temporary pause and automatic deployment remain separate controls.</Alert>
                ...
                {automationForm.deployEnabled && <FormControlLabel control={<Switch checked={automationForm.deployNewStacks} onChange={(event) => setAutomationForm({...automationForm, deployNewStacks: event.target.checked})}/>} label="Automatically authorize and deploy stacks Dockman has never deployed"/>}
                ...
                {automationForm.deployEnabled && automationForm.deployNewStacks && <Alert severity="warning">A compose.yml/docker-compose.yml that arrives from Git, or that changes in a synchronized stack Dockman has never deployed, is validated, dry-run, deployed, then retained as an authorized target. This is what brings up a service added to an existing stack. A stack excluded from automatic synchronization, or paused, stays out. At most 10 stack folders are authorized per synchronization.</Alert>}
                {automationForm.deployEnabled && <Box><Typography variant="subtitle2" sx={{mb: 1}}>Compose deployment targets</Typography><ComposePathSelector key={`${automationBinding?.id || "automation"}-deploy`} paths={automationForm.autoSyncComposePaths} selectedPaths={automationDeploySelection} onChange={(next) => setAutomationForm({...automationForm, deployComposePaths: automationForm.autoSyncComposePaths.filter((path) => next.has(path))})} selectedLabel="authorized" unselectedLabel="not authorized" maxHeight="34vh"/></Box>}
